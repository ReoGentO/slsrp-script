...
function samp.onServerMessage(color, text)
	...
	if nick3 and id3 and num then
		return {color, string.format("[A] Администратор %s[%d] перекрасил зону во цвет банды %d", nick3, id3, num)}
	end
	local nick4, id44, text4 = text:match("%[A%] Администратор (%w+_%w+)%[(%d+)%] {FFFFFF}снял лидера (.+)")
	if nick4 and id44 and text4 then
		return {color, string.format("{E14747}[A] Администратор %s[%d] снял лидера %s", nick4, id44, text4)}
	end
...
function imgui.OnDrawFrame()
	...
	if changetheme.v then
		...
		imgui.Begin(u8'Поменять тему', changetheme, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar)
		if imgui.Combo(u8"Смена темы", iStyle, colorThemes, #colorThemes) then
	...
	if remenu.v then
		...
		if isPed and doesCharExist(pPed) and isCharInAnyCar(pPed) then
			...
			imgui.Text(u8"Скорость:"); imgui.SameLine(90); imgui.Text(isPed and tostring(math.ceil(carSpeed)) or u8"Нет")
			imgui.End()
        end
		imgui.End()
	end
	...
	if giveweapon.v then
		...
		imgui.Begin(u8'Выдача оружия', giveweapon, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar)
	...
	if msetstat.v then
		...
		imgui.Begin(u8'Меню /setstat', msetstat, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar)
	...
	if ruleswindow.v then
		...
		imgui.Begin(u8'Список правил', ruleswindow, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoResize)
	...
	if fractionsmenu.v then
		...
		imgui.Begin(u8'Фракции', fractionsmenu, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar)
	...
	if blist.v then
		...
		imgui.Begin(u8'Черный список игроков SLS RP', blist, imgui.WindowFlags.NoCollapse + imgui.WindowFlags.NoMove + imgui.WindowFlags.NoResize + imgui.WindowFlags.NoScrollbar)
	...
	downloadUrlToFile(update_url, update_path, function(id, status)
		...
		updateFile = inicfg.load(nil, update_path)
            if tonumber(updateFile.info.vers) > script_vers then
				updateFile = inicfg.load(nil, update_path)
                sampAddChatMessage("Есть обновление! Версия: " .. updateFile.info.vers_text, -1)
                update_state = true
            end
